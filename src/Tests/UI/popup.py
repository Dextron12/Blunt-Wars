...
class Notify(object):
    ...
    def draw(self):
        ...
        if self.xOffest+self.popupSurface.get_size()[0] > mouse[0] > self.xOffest+(self.popupSurface.get_size()[0]-30) and self.yOffset+30 > mouse[1] > self.yOffset: # CLOSE BTN
            pygame.draw.rect(self.popupSurface, (255,0,0), (self.popupSurface.get_size()[0]-30, 0, 30, 30))
            if click[0] == 1:
                self.close = True
        pygame.draw.line(self.popupSurface, (0,0,0), ((self.width//3)-9.5, 7.5), ((self.width//3)-16, 23.5))
        pygame.draw.line(self.popupSurface, (0,0,0), ((self.width//3)-16, 7.5), ((self.width//3)-9.5, 23.5)) # CLOSE BTN
        ...
        """if (self.xOffest*2)-30 > mouse[0] > self.xOffest and self.yOffset+30 > mouse[1] > self.yOffset: # MOUSE OVER TOP BAR
            if click[0] == 1:
                #print(mouse[0] - (self.width//3))
                print(((self.xOffest - mouse[0]) - (mouse[0]-(self.width//3)))*-2, mouse[0])

                self.xOffest = (self.xOffest-mouse[0]) - (mouse[0]-(self.width//3))*-2"""


        window.blit(self.popupSurface, (self.xOffest, self.yOffset))
